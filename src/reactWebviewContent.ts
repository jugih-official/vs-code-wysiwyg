...
export function getReactWebviewContent(webview: vscode.Webview, context: vscode.ExtensionContext): string {
    return /*html*/ `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
<title>React Visual Designer</title>
<style>
:root {
    --toolbar-bg: #1e1e1e;
    --toolbar-border: #333;
...
<!-- TOP TOOLBAR -->
<div class="toolbar">
    <span class="title">React Designer</span>
...
(function() {
    ...
    // Preserved React/JSX file parts
    var jsxPrefix = '';
    var jsxSuffix = '';
    ...
    function parseReactContent(text) {
        try {
            ...
            var returnDirectMatch = trimmed.match(/return\\s+(<[\\s\\S]*?>)\\s*;?\\s*\\}/);
            if (returnDirectMatch) {
                isFragment = false;
                var beforeReturn2 = trimmed.substring(0, trimmed.indexOf(returnDirectMatch[0]));
                var afterReturn2 = trimmed.substring(trimmed.indexOf(returnDirectMatch[0]) + returnDirectMatch[0].length);
                jsxPrefix = beforeReturn2 + 'return (\\n';
                jsxSuffix = '\\n);\\n}' + afterReturn2;
                var jsxContent2 = returnDirectMatch[1].trim();
                return parseBodyElements(jsxContent2);
            }

            // Fallback: treat the whole file as JSX fragment
            isFragment = true;
            fragmentPrefix = '';
            fragmentSuffix = '';
    ...
    // =================== HTML GENERATION ===================
    function generateReactOutput() {
        var NL = '\\n';
        var output = '';

        if (isFragment) {
            // Fragment mode: just output positioned elements as JSX
            output += '<div data-designer-wrapper="true" style={{position:"relative",width:' + (canvas.offsetWidth || 800) + ',height:' + (canvas.offsetHeight || 500) + '}}>' + NL;
            output += generateControlElements(NL);
            output += '</div>' + NL;
            return output;
        }

        // Full component mode: prefix + JSX + suffix
        output += jsxPrefix;

        // Container div for absolute positioning
        var cw = canvas.offsetWidth || 800;
        var ch = canvas.offsetHeight || 500;
        output += '      <div data-designer-wrapper="true" style={{position:"relative",width:' + cw + ',height:' + ch + '}}>' + NL;
    ...
    function generateSingleElement(ctrl, indent, NL) {
        ...
        for (var pi = 0; pi < pk.length; pi++) {
            ...
            if (propKey === 'for') propKey = 'htmlFor';
            if (propKey === 'tabindex') propKey = 'tabIndex';
            if (propKey === 'readonly') propKey = 'readOnly';
            if (propKey === 'maxlength') propKey = 'maxLength';
            if (propKey === 'colspan') propKey = 'colSpan';
            if (propKey === 'rowspan') propKey = 'rowSpan';
            if (propKey === 'novalidate') propKey = 'noValidate';
            if (propKey === 'autoplay') propKey = 'autoPlay';
            if (propKey === 'autocomplete') propKey = 'autoComplete';
            // Boolean attributes
            if (propVal === 'true' && ['required', 'disabled', 'readOnly', 'multiple', 'noValidate',
                'controls', 'autoPlay', 'loop', 'muted', 'open', 'reversed', 'hidden'].indexOf(propKey) >= 0) {
                line += ' ' + propKey;
            } else {
                line += ' ' + propKey + '="' + escHtml(propVal) + '"';
            }
        }
