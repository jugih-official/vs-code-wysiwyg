...
export function getVueWebviewContent(webview: vscode.Webview, context: vscode.ExtensionContext): string {
    return /*html*/ `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
<title>Vue Visual Designer</title>
<style>
:root {
    --toolbar-bg: #1e1e1e;
    --toolbar-border: #333;
...
<!-- TOP TOOLBAR -->
<div class="toolbar">
    <span class="title">Vue Designer</span>
...
(function() {
    ...
    // Preserved Vue SFC parts
    var vueScriptBlock = '';
    var vueStyleBlock = '';
    var templateOpenTag = '<template>';
    ...
    function parseVueContent(text) {
        try {
            ...
            if (scriptMatch) {
                vueScriptBlock = scriptMatch[0];
            }

            // Extract <style>...</style> block (preserve as-is)
            vueStyleBlock = '';
            var styleMatch = trimmed.match(/<style[^>]*>[\\s\\S]*?<\\/style>/i);
            if (styleMatch) {
                vueStyleBlock = styleMatch[0];
            }

            // Extract <template ...> opening tag
            templateOpenTag = '<template>';
            var templateOpenMatch = trimmed.match(/<template([^>]*)>/i);
            if (templateOpenMatch) {
                templateOpenTag = '<template' + templateOpenMatch[1] + '>';
            }

            // Extract <template>...</template> content
            var templateContent = '';
            var templateMatch = trimmed.match(/<template[^>]*>([\\s\\S]*?)<\\/template>/i);
            if (templateMatch) {
                isFragment = false;
                templateContent = templateMatch[1];
            } else {
                // No <template> tag found — treat entire content as template fragment
                isFragment = true;
                fragmentPrefix = '';
                fragmentSuffix = '';
            ...
            return parseBodyElements(templateContent.trim());
        } catch(e) {
            console.error('Vue parse error', e);
            return null;
        }
    }
    ...
    // =================== HTML GENERATION ===================
    function generateVueOutput() {
        var NL = '\\n';
        var output = '';

        if (isFragment) {
            // Fragment mode: just output positioned elements
            output += '<div data-designer-wrapper="true" style="position:relative;width:' + (canvas.offsetWidth || 800) + 'px;height:' + (canvas.offsetHeight || 500) + 'px;">' + NL;
            output += generateControlElements(NL);
            output += '</div>' + NL;
            return output;
        }

        // Vue SFC mode: <template> + <script> + <style>
        output += templateOpenTag + NL;
        ...
        if (vueScriptBlock) {
            output += NL + vueScriptBlock + NL;
        }

        if (vueStyleBlock) {
            output += NL + vueStyleBlock + NL;
        }
    ...
    window.syncToVue = function() {
        var vueOutput = generateVueOutput();
        vscode.postMessage({ type: 'updateVue', content: vueOutput });
    };
    ...
    window.addEventListener('message', function(event) {
        ...
        switch (message.type) {
            ...
            case 'documentUpdate': {
                if (isDraggingControl || isResizing || isDraggingNew || autoSyncTimer) break;
                var parsed = parseVueContent(message.content);
                if (parsed) {
