...
export class ReactDesignerProvider implements vscode.CustomTextEditorProvider {
    ...
    public async resolveCustomTextEditor(
        ...
        // Set initial content
        webviewPanel.webview.html = getReactWebviewContent(webviewPanel.webview, this.context);
        ...
        const selectionChangeSubscription = vscode.window.onDidChangeTextEditorSelection((e: vscode.TextEditorSelectionChangeEvent) => {
            ...
            if (e.textEditor.document.uri.toString() === document.uri.toString() && webviewPanel.visible) {
                ...
                if (match) {
                    const nameMatch = lineText.match(/(?:id|className)=["'{]([^"'}]+)/);
                    ...
                    for (let i = 0; i < line; i++) {
                        ...
                        if (prevMatch && prevMatch[1] === elemType) {
                            const prevNameMatch = prevLine.match(/(?:id|className)=["'{]([^"'}]+)/);
                ...
                case 'selectElement': {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        ...
                        if (line.indexOf('<' + elemType) >= 0) {
                            const nameMatch = line.match(/(?:id|className)=["'{]([^"'}]+)/);
